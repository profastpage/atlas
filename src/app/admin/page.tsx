...
const ALLOWED_ADMIN_EMAIL = 'profastpage@gmail.com';
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (userEmail !== ALLOWED_ADMIN_EMAIL) {
        setLoginError('Acceso denegado. Solo el Super Admin puede ingresar con Google.');
        return;
      }

      if (!idToken) {
        setLoginError('No se obtuvo el token de Firebase.');
        return;
      }

      // Exchange Firebase token for Turso auth
      ...
      if (!res.ok) {
        setLoginError(data.error || 'Error al autenticar con Google.');
        return;
      }

      if (!data.user?.isAdmin) {
        setLoginError('Acceso denegado. La cuenta no tiene permisos de administrador.');
        return;
      }
      ...
      setLoginError('Error de conexion con Google.');
    } finally {
      ...
      <div className="flex flex-col items-center justify-center min-h-dvh bg-gray-950 px-6">
        ...
        >
          ...
          <div className="bg-gray-900/80 backdrop-blur-md rounded-2xl border border-gray-800/50 p-6 shadow-2xl">
            {/* Google Login — restricted to profastpage@gmail.com */}
            <button
              type="button"
